...
internal static class MacOSUsbAPI
{
    ...
    [DllImport(IOKit)]
    public static extern int IORegistryEntryGetRegistryEntryID(IntPtr entry, out ulong entryID);
    ...
    public static bool TryQueryInterface(IntPtr pluginInterface, out IntPtr result, params Guid[] interfaceIds)
    {
        result = IntPtr.Zero;
        if (pluginInterface == IntPtr.Zero || interfaceIds.Length == 0) return false;

        var queryInterface = GetDelegate<QueryInterfaceDelegate>(pluginInterface, Offset_Plugin_QueryInterface);
        foreach (var iid in interfaceIds)
        {
            if (queryInterface(pluginInterface, iid, out result) == 0 && result != IntPtr.Zero)
            {
                return true;
            }
        }
