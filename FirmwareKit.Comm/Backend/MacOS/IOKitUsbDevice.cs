...
/// </summary>
/// <remarks>
/// The COM-vtable calling shape and offsets follow SharpFastboot's macOS backend
/// (verified to work on current macOS): plug-in interface at
/// <c>Offset_Plugin_QueryInterface/Release</c>, device interface vtable slots
/// <c>Offset_USBDeviceOpen/USBDeviceClose/USBGetDeviceVendor/USBGetDeviceProduct/
/// USBGetSerialNumberStringIndex/USBGetConfiguration/USBSetConfiguration/
/// USBDeviceCreateInterfaceIterator/USBDeviceReset</c>, and interface vtable
/// slots <c>Offset_USBInterfaceOpen/USBInterfaceClose/GetNumEndpoints/
/// GetPipeProperties/ClearPipeStallBothEnds/ReadPipeTO/WritePipeTO</c>.
/// <para>COM-vtable 调用形与偏移遵循 SharpFastboot 的 macOS 后端（在当前 macOS
/// 上验证可用）：插件接口在 <c>Offset_Plugin_QueryInterface/Release</c>，设备接口
/// vtable 槽 <c>Offset_USBDeviceOpen/USBDeviceClose/USBGetDeviceVendor/
/// USBGetDeviceProduct/USBGetSerialNumberStringIndex/USBGetConfiguration/
/// USBSetConfiguration/USBDeviceCreateInterfaceIterator/USBDeviceReset</c>，接口
/// vtable 槽 <c>Offset_USBInterfaceOpen/USBInterfaceClose/GetNumEndpoints/
/// GetPipeProperties/ClearPipeStallBothEnds/ReadPipeTO/WritePipeTO</c>。</para>
/// </remarks>
    ...
    public override int CreateHandle()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var openDev = IOKitUsbAPI.GetDelegate<IOKitUsbAPI.USBDeviceOpenDelegate>(_deviceInterface, IOKitUsbAPI.Offset_USBDeviceOpen);
                kr = openDev(_deviceInterface);
                UsbTrace.Log($"IOKitUsbDevice.CreateHandle: USBDeviceOpen kr={kr} (0x{kr:X})");
                if (kr != IOKitUsbAPI.S_OK) return -1;

                // Align with MacHostUsbDevice: ensure Configuration 1 is selected
                // before creating the interface iterator. Best-effort; a device may
                // legitimately have a single, differently numbered configuration.
                // <para>与 MacHostUsbDevice 对齐：在创建接口迭代器前确保选择配置 1。
                // 尽力而为；设备可能仅有一个编号不同的配置。</para>
                try
                {
                ...
                {
                    bInterfaceClass = IOKitUsbAPI.kIOUSBFindInterfaceDontCare,
                    bInterfaceSubClass = IOKitUsbAPI.kIOUSBFindInterfaceDontCare,
                    bInterfaceProtocol = IOKitUsbAPI.kIOUSBFindInterfaceDontCare,
